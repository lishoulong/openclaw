...
export type SpawnSubagentContext = {
  ...
  /**
   * Internal escape hatch: allow orchestrators (e.g. teams) to spawn cross-agent subagents
   * without requiring the caller's `subagents.allowAgents` to whitelist every member.
   */
...
export async function spawnSubagentDirect(
  ...
  // Cross-agent spawns are usually gated by allowAgents. Internal orchestrators may bypass.
  if (targetAgentId !== requesterAgentId && ctx.skipAllowlist !== true) {
